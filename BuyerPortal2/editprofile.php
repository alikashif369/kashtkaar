<?php
    ...
    // Check if session exists instead of starting a new one
    ...
    while($row = mysqli_fetch_array($run_query))
    {
        ...
        $cnic = $row['buyer_cnic']; // Updated variable
        $bank = $row['buyer_bank'];
    ...
    // Add this after getting user data
    if(isset($_POST['confirm'])) {
        ...
        $updated_cnic = mysqli_real_escape_string($con, $_POST['cnic']);
        $updated_bank = mysqli_real_escape_string($con, $_POST['bank']);
        $updated_comp = mysqli_real_escape_string($con, $_POST['company']);
        $updated_license = mysqli_real_escape_string($con, $_POST['license']);
        $updated_email = mysqli_real_escape_string($con, $_POST['email']);

        $update_query = "UPDATE buyerregistration SET 
            buyer_name='$updated_name',
            buyer_phone='$updated_phone',
            buyer_addr='$updated_address',
            buyer_cnic='$updated_cnic',
            buyer_bank='$updated_bank',
            buyer_comp='$updated_comp',
            buyer_license='$updated_license',
            buyer_mail='$updated_email'
            WHERE buyer_phone='$sessphonenumber'";

        if(mysqli_query($con, $update_query)) {
            echo "<script>alert('Profile Updated Successfully!')</script>";
            echo "<script>window.location.href='BuyerProfile.php'</script>";
        } else {
        ...
        <form action="" method="POST">
            <div class="form">
                ...
                <div class="input-group mt-4 s">
                    ...
                    <input type="tel" name="phone" class="form-control border border-dark" id="staticEmail" value="<?php echo $phone?>">
                </div>
                <div class="input-group mt-4 s">
                    <div class="input-group-prepend">
                ...
                <div class="input-group mt-4 s">
                    <div class="input-group-prepend">
                        <span class="input-group-text text" id="inputGroup-sizing-default"><i class="fas fa-pencil-alt mr-2"></i>CNIC No.</span>
                    </div>
                    <input type="text" name="cnic" class="form-control border border-dark" id="staticEmail" value="<?php echo $cnic?>">
                </div>
                <div class="input-group mt-4 s">
                    <div class="input-group-prepend">
                        <span class="input-group-text text" id="inputGroup-sizing-default"><i class="fas fa-university mr-2"></i>Account No.</span>
                    </div>
                    <input type="text" name="bank" class="form-control border border-dark" id="staticEmail" value="<?php echo $bank?>">
                </div>
                <div class="input-group mt-4 s">
                    <div class="input-group-prepend">
                        <span class="input-group-text text" id="inputGroup-sizing-default"><i class="fas fa-building mr-2"></i>Company</span>
                    </div>
                    <input type="text" name="company" class="form-control border border-dark" id="staticEmail" value="<?php echo $comp?>">
                </div>
                <div class="input-group mt-4 s">
                    <div class="input-group-prepend">
                        <span class="input-group-text text" id="inputGroup-sizing-default"><i class="fas fa-id-badge mr-2"></i>License</span>
                    </div>
                    <input type="text" name="license" class="form-control border border-dark" id="staticEmail" value="<?php echo $license?>">
                </div>
                <div class="input-group mt-4 s">
                    <div class="input-group-prepend">
                        <span class="input-group-text text" id="inputGroup-sizing-default"><i class="far fa-envelope mr-2"></i>Email id</span>
                    </div>
                    <input type="email" name="email" class="form-control border border-dark" id="staticEmail" value="<?php echo $mail?>">
                </div>
            </div>
